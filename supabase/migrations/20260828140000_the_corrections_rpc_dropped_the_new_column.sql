-- THE FOURTH APPEARANCE OF ONE DEFECT CLASS THIS WEEK: a hand-built object
-- that silently drops any field it was not taught. advanceProgress dropped
-- new RefreshProgress fields; the coverage fallback dropped the five live
-- figures; vHead lacked tracked; and now apply_posting_corrections — the
-- batched patch path every RE-INGESTED row goes through — hand-lists seven
-- columns, so the employment_type the nine vendor parsers started emitting
-- was applied on brand-new INSERTS and silently dropped from every patch.
-- Measured live: hours after the .47 deploy, THREE typed rows corpus-wide
-- (the inserts), zero from the ~4,500 boards rotation had already re-visited.
--
-- Same CASE-on-key-presence contract as the existing columns: a patch that
-- does not mention employment_type leaves the stored value untouched, so the
-- edge's stated-only rule (vendor silence never erases a value) is preserved.
