-- search_jobs v3 (two-tier): broad terms still timed out under v2.2 — the
-- GIN recheck step must detoast every candidate's big description-laden
-- vector before ranking can even start (measured: rare terms 0.5-0.7s,
-- broad terms >8s regardless of bounding). v3 splits the work:
--   Tier 1: match + rank on a SMALL title/company/department vector
--           (title_tsv) — cheap for any term; for broad queries,
--           description mentions carry no ranking signal anyway.
--   Tier 2: only when tier 1 finds fewer than 200 matches (niche terms),
--           extend matching into full descriptions via search_tsv — the
--           measured-fast case (few matches to detoast).
-- Semantics stay honest: broad searches rank title/company relevance;
-- niche searches also surface description-only mentions.
...
CREATE INDEX IF NOT EXISTS job_board_postings_title_tsv_idx
  ON public.job_board_postings USING gin (title_tsv);
...
BEGIN
  ...
  IF title_total < 200 THEN
    ...
    EXECUTE 'SELECT count(*) FROM (SELECT 1 FROM public.job_board_postings p WHERE p.search_tsv @@ $1' || filters || ' LIMIT 10000) c'
      INTO total
      USING q, p_fresh_cutoff, p_location, p_country, p_category, p_experience, p_salary_floor, p_companies, p_posted_after, p_max_age_days;
  ELSE
    total := title_total;
  ...
  RETURN QUERY EXECUTE
    ...
    || 'FROM public.job_board_postings p WHERE ' || tsv_col || ' @@ $1' || filters
    || ' ORDER BY ts_rank_cd(' || tsv_col || ', $1) DESC, p.effective_posted DESC, p.id ASC'
