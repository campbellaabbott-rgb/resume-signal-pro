-- search_jobs v3.1: tier 2 samples instead of exhaustively matching. The
-- middle case (term rare in titles, common in descriptions — 'kubernetes')
-- still timed out: tens of thousands of big-vector rechecks. Tier 2 now
-- collects up to 3,000 description matches (recheck stops there) and ranks
-- the sample on the small vector + recency; big vectors are never ranked.
-- Reported count is capped at the sample bound (3,000) for tier-2 queries.
...
BEGIN
  ...
  IF title_total < 200 THEN
    -- Tier 2 (niche term): matching on the big description vector is only
    -- affordable as a bounded SAMPLE — collect up to 3,000 matches without
    -- ordering (stops after 3k recheck passes), then rank the sample on the
    -- SMALL vector + recency. Big vectors are never detoasted for ranking.
