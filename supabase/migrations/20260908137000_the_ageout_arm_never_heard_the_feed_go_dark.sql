-- AN UNCENSORED NUMERATOR OVER A CENSORED DENOMINATOR.
--
-- get_company_fill_curve returns fills_90d, relists_90d and ageouts_90d, and
-- /explore's "still advertised when it crossed day 30" answer divides the
-- third by the sum of all three. Two of those three were censored for
-- feed-dark collection failures. The third was not.
--
-- In the closure arm of `raw`, is_fill and is_relist are zeroed whenever
--     COALESCE(c.suspect, false) OR dk.tok IS NOT NULL
-- -- the collector's own stamp, plus the retroactive proxy over unstamped
-- history. The exit-ledger arm carried neither: it has no suspect column, it
-- joined no `dark`, and is_ageout was set on exit_reason alone.
--
-- SO THE SHARE WAS A RATIO OF TWO POPULATIONS, AND IT ERRED TOWARD THE
-- ACCUSATION. Take an employer with 100 fills, 60 relists and 40 age-outs in
-- the window, one unstamped batch tripping the proxy and censoring 50 fills
-- and 30 relists. Published: 40 / (40 + 50 + 30) = 33%. True: 40 / 200 = 20%.
-- The card leads with that figure at 3xl under a heading naming employer
-- conduct, and its own methodology asserted "all counts from the same
-- measurement under the same feed-dark policy, so the ratio is of one
-- population rather than two". Both halves of that sentence were false of the
-- numerator, and the error ran the one direction a conduct claim may not err.
--
-- THE FIX IS EXACT, NOT AN APPROXIMATION, because the two ledgers share a
-- batch key. One board pass computes a single `closedAt` and writes it as
-- job_board_closures.closed_at on the takedowns of that pass and as
-- job_board_exits.exited_at on the age-outs of the SAME `vanished` set
-- (job-board/index.ts: the aged-exit mapper and the closure-row mapper are
-- both inside that loop and both stamp `closedAt`). So (company_token,
-- exited_at) is the closure batch key itself. `bad_batch` collects the batches
-- the closure arm already refuses to count -- suspect-stamped, or dark by the
-- proxy -- and the exit arm zeroes is_ageout on them.
--
-- CENSORED, NOT DELETED, for the reason this function's own header gives: the
-- postings almost certainly did not go anywhere, we stopped being able to see
-- them. The exit rows stay in the risk set at tt and stay in dated_coverage;
-- they simply stop being EVENTS. A false positive costs one observation, not
-- an employer's whole age-out record.
--
-- WHAT REMAINS UNCENSORABLE, SAID PLAINLY: a pass in which every vanished
-- posting aged out writes no closure row at all, so it has no suspect stamp
-- and no batch to size, and its age-outs cannot be judged from here. That
-- residual over-counts age-outs -- the accusing direction -- and is bounded by
-- how rarely a whole pass is age-outs alone. It is stated in the COMMENT ON
-- rather than left for a reader to discover.
--
-- NOTHING ELSE CHANGES. The estimator, the cohort-on-origin selection, the
-- clamp at 31, the censoring set, `sufficient`, every bound and every column
-- is byte-for-byte 20260906091000. The guard that pins get_company_fill_curve
-- to a file moves to this one in the same commit -- its own comment already
-- established that the pin follows the function to the definition the database
-- actually runs, rather than asserting a file that has stopped running.
...
-- the PUBLIC pseudo-role removes nothing a named grant holds.
