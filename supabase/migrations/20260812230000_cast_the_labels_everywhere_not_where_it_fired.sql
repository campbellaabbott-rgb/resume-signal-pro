-- THE SAME LATENT HANDLER BUG, CLOSED EVERYWHERE IT EXISTS — NOT WHERE IT FIRED.
--
-- 19:05, first post-fix tick with a firing handler in refresh_ghost_stats:
--
--     ERROR: malformed array literal: "counts"  LINE 1: stale := stale || 'counts'
--
-- An unknown-typed literal on || resolves toward anyarray || anyarray, so the
-- label parses as an array literal and THE HANDLER ITSELF dies. The bug was
-- unreachable for three days because handlers could never fire (WHEN OTHERS
-- does not catch QUERY_CANCELED); the moment they could, it surfaced. Lovable
-- cast the three ghost labels and reasoned the other two functions were on an
-- "already-typed path" — but their concatenations are textually identical, and
-- explore missed its 19:07 tick right after a slow section would have fired a
-- handler there for the first time. Succeeding stats runs prove only that
-- their handlers did not fire on those ticks, not that firing is safe.
--
-- So: every stale-label concatenation in refresh_stats_cache and
-- refresh_explore_cache now carries ::text. Redundant where the resolver
-- happens to pick anyelement; fatal where it does not; uniform either way.
--
-- AND refresh_transparency_cache GETS THE HANDLERS IT SHIPPED WITHOUT. It was
-- written this afternoon, hours before the QUERY_CANCELED lesson, with no
-- wrapping at all — one slow callee (get_pay_transparency measured 14.9s
-- against its 25s budget) kills the run whole, which is why the cache still
-- shows its 17:47 prime after the 18:37 tick. Same treatment as its siblings:
-- both sections degrade to the previous row's value, both arms, stale_parts
-- says what degraded, the write is unconditional.
