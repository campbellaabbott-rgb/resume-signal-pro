-- THE SECTION RENDERS ITS REASON WHEN IT CANNOT RENDER ITS NUMBER.
--
-- The Ghost Job Index reader for job_board_layoff_partition. It returns TWO
-- ROWS, always: the filed arm and the control arm, from a LEFT JOIN off
-- the two arm names, so an arm the writer has not written yet is a row with
-- lp_sufficient_30 false and a reason, never a missing row. The page prints
-- the sentence only when BOTH rows are sufficient and otherwise prints
-- "No reading yet: {reason}" with the n and employer count the row carries,
-- which is a fact about our sample and not about any employer.
--
-- THE REASON VOCABULARY, one word per key the page holds:
--   n           fewer than {minN} roles in that arm reached the 30-day cap
--   width       the interval is wider than +/- {maxHw} points
--   employers   fewer than {minEmployers} employers with a qualifying filing
--               have roles on boards we read to the end (filed arm only)
--   share       one employer holds more than {maxShare} of that arm's roles
--               (filed arm only)
--   stale       the reading has not been recomputed since {computedAt} --
--               decided HERE from computed_at against the WARN stale bound,
--               so a writer that stopped running cannot keep printing a
--               sentence it computed a week ago; it also covers a row the
--               writer never wrote
--   arithmetic  R + X + S did not sum to one; a defect, never a market fact
--
-- lp_separated is the same on both rows: true when the two S(30) intervals
-- do not overlap, false when they do, NULL when either arm lacks one. It is
-- computed and printed, never gated on: an overlap is a finding. Nothing
-- here divides one arm by the other.
--
-- Every threshold the page prints comes back on the row (lp_min_n,
-- lp_max_half_width, lp_min_employers, lp_max_employer_share, the stale
-- bound) so the copy's placeholders can be checked against what the server
-- actually gated on; the client renders them from src/config/layoffs.ts,
-- and the cross-runtime guard keeps the two equal.
