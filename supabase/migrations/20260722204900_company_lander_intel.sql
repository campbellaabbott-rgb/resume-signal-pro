-- Company-lander intel + Explore drill-through. All computed from data we
-- already store; every figure names its basis, absent data renders nothing.

-- 1) One-call lander intel: verified headcount (with basis), stated-salary
--    median for THIS company, top categories, top countries, and 7-day
--    net-new trend from the daily snapshots (same definition as Explore
--    trending: today minus oldest snapshot within 7d; NULL until 2 snapshot
--    days exist — no fabricated history).
...
AS $$
  ...
  toks AS (
    -- All board tokens of the same company slug (multi-board companies:
    -- gilead has two Workday tenants, etc.)
...
-- 2) Similar companies: same dominant field, actively listed, not excluded,
--    not the same company slug. Ranked by open roles; carries headcount when
--    known so the UI can show the same sourced badge.
...
-- 3) Segment drill-through: the full verified-headcount company list for one
--    band (the Explore top-12 is a teaser; this is the "see all N" payload).
--    Same population and gates as get_size_segments v5.
...
-- 4) Claim status now also returns the verified claimant's stated website —
--    displayed on the lander labeled as employer-provided. Identity fence
--    unchanged: nothing here touches computed data.
