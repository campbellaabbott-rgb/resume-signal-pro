-- A FILING IS PRUNED ONLY AFTER ITS MONTH IS COUNTED.
--
-- The retention writer, in the closure log's shape (20260727140000): roll
-- each month of filings older than the keep window into layoff_filing_rollup
-- (filings and stated workers per month, source and state), then delete
-- only rows whose month is provably in the rollup. Never a bare DELETE cron.
-- The keep window floors at 180 days because the partition needs a 90-day
-- cohort plus a 90-day lookback of filings behind it; the default is the
-- spec's 365. Match rows go with their filing (ON DELETE CASCADE); an
-- amendment that pointed at a pruned ancestor keeps its own row with
-- supersedes_id set NULL. The read log is trimmed to 90 days in the same
-- call; it is a heartbeat trace, not the moat, and needs no rollup.
