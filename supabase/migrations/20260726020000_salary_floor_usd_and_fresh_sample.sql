-- Two search-path integrity fixes (2026-07-26 upgrade batch), same functions:
--
-- 1) SALARY FLOOR BINDS CURRENCY. The floor filter compared raw
--    salary_min_annual with no currency predicate, so the "$100,000+" filter
--    passed a SEK 1,000,000 (~$95k) or JPY 10,000,000 (~$66k) row — numbers
--    that only look like they clear the bar. The board already solved this
--    exact class for SORTING with salary_rank_usd (approximate-FX generated
--    column; NULL when currency is unidentified). The filter now uses the same
--    column: a row passes a $ floor only when its stated pay actually clears
--    it in approximate USD terms. Rows with unidentifiable currency have a
--    NULL rank and cannot pass — consistent with the filter's existing promise
--    ("states pay at or above the floor"), where unverifiable is a miss, not
--    a pass. EUR/GBP high earners now pass correctly instead of only
--    lexically-large numbers.
--
-- 2) TIER-2 SAMPLE PREFERS FRESH. The description tier's candidate sample
--    (`LIMIT 3000` with no ORDER BY) took whatever heap order supplied, which
--    skews toward long-resident rows — so on niche queries, a posting from
--    TODAY could be invisible while 29-day-old rows filled page one. The
--    sample now takes the 3,000 FRESHEST matches (effective_posted DESC).
--    Ranking within the sample is unchanged (title-rank, then recency).
--    The existence-count keeps its unordered LIMIT — counting needs any 3,000,
--    only the sample users see needs the right 3,000.
...
BEGIN
  ...
  IF tsv_col = 'p.search_tsv' THEN
    -- Tier 2 (sampled): the 3,000 FRESHEST matches, ranked on the small
...
-- count_jobs_capped: the same currency fix, so the count describes the same
-- set the page shows.
