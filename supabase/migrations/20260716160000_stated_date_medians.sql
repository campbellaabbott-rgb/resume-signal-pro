-- The Ghost Job Index's "median age of an open posting" measured DISCOVERY age
-- (now() - first_seen), not posting age: after census merges added thousands
-- of boards at once, the median collapsed to ~2.8 days — visibly implausible,
-- and not what the label claims. The per-company hiring-health medians had the
-- same basis bug via COALESCE(posted_at, first_seen), which let recently
-- discovered boards masquerade as fast fillers (false "typically fills in ~Xd"
-- urgency chips).
--
-- Fix: every age / time-to-fill median now uses ONLY the company's own stated
-- posted_at. Rows without a stated date are excluded, never approximated, and
-- the index RPC reports the measured share of postings that state a date so
-- the page can disclose its basis.
...
AS $$
  SELECT
    ...
    -- Median age by the company's own stated date. TABLESAMPLE keeps the
    -- percentile fast at ~186k rows (honest estimate, exact counts elsewhere).
