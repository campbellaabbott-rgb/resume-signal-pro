-- A SINGLE WORD IS NOT AN EMPLOYER.
--
-- The matcher. It is the only writer of layoff_matches, and layoff_matches
-- is the only way a filing reaches a board, so the rules here ARE the
-- never-surface rule (SPEC section 5). Every run deletes the table and
-- rewrites it for filings whose event_date is inside the last 365 days.
--
-- RULE 1, ALIAS. An accepted layoff_employer_aliases row whose alias_norm
-- equals the filing's filer_norm (WARN rows, with state_scope NULL or
-- naming the notice's state) or whose cik equals the filing's cik (SEC
-- rows; never a ticker). matched_via alias, relation copied from the row.
--
-- RULE 2, EXACT MULTI-TOKEN. The filer_norm has two or more tokens of two
-- or more letters (a possessive "s" or a dotted "com" is punctuation, not a
-- word: Kohl's, Macy's and Amazon.com are single-word brands) AND equals the
-- mirrored display_norm of exactly ONE employer AND, for a WARN row, that
-- board has a live posting in the notice's state (job_board_postings stores
-- region_code with its country prefix, so the gate compares against
-- 'US-' || state). An SEC filer is national and skips the gate. A pair a
-- person REJECTED in the alias table is refused here whatever the names say.
--
-- WHO IS ONE EMPLOYER. The employer key of a token is its first '~' segment
-- -- the tenant on workday and oracle, the slug itself elsewhere -- EXCEPT
-- where that segment is a vendor path shared by every client: UKG tokens
-- are recruiting~<CLIENT>~<uuid> / recruiting2~<CLIENT>~<uuid> (1,291
-- boards under two segments), and the EU pods of greenhouse and lever are
-- eu~<slug> (221 boards under one). There the key is the client segment or
-- the whole token. Without that, two UKG clients sharing a display name
-- would count as one employer and a filing would be written to both.
--
-- RULE 3, REFUSED AND COUNTED: every single-token norm (23 of the 41 WARN
-- employers of the last 90 days; 486 of 803 SEC hits; the class that gave
-- Emerson -> Emerson College, Wise Company -> the fintech, Mosaic Company ->
-- a startup, Block -> a law firm), every norm that names two employers
-- (Owens Corning and Owens & Minor share a board norm), every WARN
-- multi-token match that fails the state gate. The counts come back in the
-- row this function returns so the poller's log line and the candidate
-- script can read them; a refused filing gets no row and reaches no surface.
--
-- RULE 4, NEVER: trigram, Levenshtein, prefix, ticker equals token, slug
-- title-casing, hiringOrganization.name. None of those appear below and the
-- behavioural guard (lane F) plants the seven verified wrong pairs and
-- asserts zero rows for them.
--
-- The match rows are written for every status so an amendment can be
-- resolved to its original later; the READERS gate on status, form,
-- workers, the display window and the read date. The matcher decides WHO
-- the filer is; the reader decides WHAT may be printed.
