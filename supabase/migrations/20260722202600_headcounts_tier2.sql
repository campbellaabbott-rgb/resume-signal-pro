-- Tier-2 headcount harvest: 35 verified employee counts for major boards,
-- from Wikidata P1128 restricted to (a) DATED statements only, (b) entities
-- whose direct P31 type is on a company-type whitelist (business/enterprise/
-- company/public company/bank/airline/retail chain/corporation/tech/software),
-- (c) unambiguous single-entity label matches, (d) board-consistency
-- (open postings must not exceed 1.2x claimed employees), and (e) a manual
-- cross-check of every row against documented reality. Rejected in that final
-- pass: Novartis (matched subsidiary entity, 1,679 vs ~76k real), Debeka
-- (436 vs ~16k), Edwards (surname ambiguity), Tomra (408 vs ~5k), Roche
-- (215 vs ~100k - same trap the Tier-1 audit caught).
--
-- ON CONFLICT DO NOTHING: hand-curated Tier-1 rows always win.
...
-- AutoStore's 370 (Tier-1) shares an as-of date (2026-05-12) with Tomra's
-- provably-wrong 408 - same suspicious batch edit, and it contradicts
-- AutoStore's documented ~1,100. Nothing rather than a guess.
DELETE FROM public.company_profiles WHERE company_token = 'autostore~wd3~autostore';

-- get_size_segments v5: same headcount-only banding as v4, plus legal-suffix
-- normalization in the name dedupe so "The Quality Group GmbH" and
-- "The Quality Group" collapse into one row (shortest variant displays).
