...
--
-- 1) search_jobs v3.4 — the ranked path silently DROPPED the work-mode filter
--    and returned a narrower column set than the recency path.
--
--    Measured: workMode=remote + q=engineer returned 30 rows of which 30 had
--    work_mode NULL — that is, postings which state no work mode at all, the
--    exact opposite of what the filter asks for. The filter simply had no
--    parameter to travel through. Filtering alone was not the whole story:
--    RETURNS TABLE carried no work_mode and no structured salary columns
--    either, and rowToJob reads all five, so on ANY search the work-mode badge
--    and the salary range silently disappeared from every card. The same
--    posting rendered differently depending on whether the user had typed a
--    query.
--
--    p_work_mode is inlined via quote_literal rather than a new placeholder so
--    the existing $1..$13 numbering (which differs between the count EXECUTEs
--    and the page EXECUTEs) does not have to be renumbered. It is constrained
--    to the three legal values before it is ever concatenated.
--
-- 2) count_jobs_capped — filter latency. The exact count was costing 3-9s on
--    every broad filter while the page itself took ~0.3s, and on the middle
--    band (~150-190k rows) it exceeded the statement timeout entirely. Capping
--    the count is the same trick search_jobs already uses internally
--    (SELECT count(*) FROM (SELECT 1 ... LIMIT n)): it stops early, so cost is
--    bounded by the cap instead of by how many rows match. Below the cap the
--    number is exact; above it the caller shows "10,000+", which is true
--    rather than a guess.
...
BEGIN
  ...
  -- Constrained to the three legal values before concatenation; anything else
...
-- ── Capped count for the non-search (recency) path ────────────────────────
-- Mirrors serveList's filter set exactly. Returns the exact count when the
-- matching set is smaller than the cap, otherwise the cap with capped=true so
-- the caller can render "10,000+" instead of inventing a number.
...
BEGIN
  ...
  IF p_work_mode IN ('remote', 'hybrid', 'onsite') THEN
    filters := filters || ' AND p.work_mode = ' || quote_literal(p_work_mode);
  END IF;
  -- Free-text on this path is the recency path's OR-of-ILIKE, kept identical
  -- so the count matches the page it describes.
