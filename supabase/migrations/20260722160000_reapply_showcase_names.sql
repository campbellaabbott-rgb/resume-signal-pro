-- RE-APPLY under a fresh timestamp: probes on 2026-07-22 proved migrations
-- 20260721320000 and 20260721330000 never took effect in production (raw
-- 'Tysonfoods'/'Campingworld' names, showcase_excluded absent from the API
-- schema, globalelitecareers alive and regrown to 1,815 postings on the
-- still-undeployed .4 function bundle). Runner rule reaffirmed: NEVER trust a
-- deploy round — probe every artifact. Everything below is idempotent.
-- The 330000 content includes its (v2) get_size_segments; the v3 definition
-- from 20260721340000 is re-asserted afterwards so headcount banding wins.

-- ============ content of 20260721320000 (names batch 5) ============
...
-- ============ content of 20260721330000 (showcase integrity) ============
...
-- ============ re-assert segments v3 (headcount banding, from 340000) ============
CREATE OR REPLACE FUNCTION public.get_size_segments()
RETURNS jsonb
LANGUAGE sql STABLE SECURITY DEFINER
SET search_path = public
...
-- Fresh purge: the spam board regrew while the .5 bundle sat undeployed.
DELETE FROM public.job_board_postings WHERE company_token = 'globalelitecareers';
DELETE FROM public.job_board_closures WHERE company_token = 'globalelitecareers';
DELETE FROM public.job_board_company_snapshots WHERE company_token = 'globalelitecareers';
...
-- PostgREST schema reload so new tables are visible to the API immediately.
