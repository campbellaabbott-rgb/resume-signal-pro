-- The search path could not return a posting's country, so every row served by
-- a SEARCH (as opposed to a plain list) carried country: null.
--
-- search_jobs FILTERS on p.country and always has — the country filter's recall
-- is exact — but the function never PROJECTED it, and the edge function's
-- rowToJob maps RPC rows through the same shape as list rows. So the field I
-- added to the mapper this morning resolved to undefined on the search path.
--
-- Found by live verification, not by the test suite, and the gap is worth
-- naming: the unit test asserted that rowToJob EMITS country. It passed. It
-- could not see that no query ever fetches the column, because a mapper cannot
-- invent a field the SELECT did not ask for. The list path had the same defect
-- (its explicit column list omitted country) and is fixed in the same commit.
--
-- Adding a column to RETURNS TABLE cannot be done with CREATE OR REPLACE, so
-- this DROPs and recreates. The parameter list is byte-identical to the live
-- one — verified mechanically, not by eye — so this replaces rather than
-- overloads. Everything else, including the title_hits union that made exact
-- title matches reachable again, is carried through unchanged.
