...
--
--    days_on_board stopped being a coalesce at the four collector write sites
--    on 2026-09-06 (20260906214000): each row now names its clock in
--    origin_basis. That fixes the ROW. It does not fix a SUMMARY that folds
--    the two clocks into one percentile — and this summary is the one place
--    where the mistake becomes irreversible, because step 2 deletes the raw
--    rows a reader would otherwise use to take the number apart again.
--
--    CHOSEN: carry origin_basis, in the COLUMNS rather than in the key. Every
--    duration statistic is emitted once per basis with its own denominator
--    (n_stated/p50_days_stated/p75_days_stated, n_discovered/p50_days_
--    discovered/p75_days_discovered), and the pre-stamp rows are counted in
--    n_basis_unrecorded with NO percentile at all. Volume, pay disclosure and
--    dim_counts stay at the unsegmented grain, because those are facts about
--    the employer's board and not about which dates the feed happened to
--    carry. This is the same shape roll_up_and_prune_closures was moved to on
--    the same day (20260906093000): the percentile is restricted by FILTER to
--    the population that can honestly support it, and the size of that
--    population is published beside it as dated_n. Two summaries that will be
--    read side by side now disclose their coverage the same way, and neither
--    coalesces.
--
--    REJECTED — FILTER THE WHOLE ROLLUP TO origin_basis = 'stated'. Simpler,
--    and it matches the estimator's dated-cohort restriction
--    (20260906091000), which is the argument for it. It fails on this table
--    for a reason the estimator never faces: the estimator DROPS undated rows
--    from a live query, so nothing is lost — the rows are still there
--    tomorrow. Here the prune matches on the KEY, so a group that exists in
--    the rollup because it contained one 'stated' row would have its
--    'discovered' and pre-stamp rows deleted by step 2 while being summarised
--    nowhere. That is precisely the delete-before-summarise shape this file
--    exists to end, applied to the subset least able to survive it. It would
--    also silently empty the ghost-rate surfaces that already read this
--    ledger for undated boards: age-outs are the ghost-rate NUMERATOR, and an
--    undated posting that aged out is a ghost whatever clock measured it.
--
--    REJECTED — origin_basis IN THE PRIMARY KEY. It keeps both series too,
--    but it changes the SHAPE of every other number here: exits, salary
--    disclosure and dim_counts would be split by a property of OUR metadata,
--    so "how many roles ended in Colorado in July" becomes a sum across
--    basis rows that a reader has to remember to perform, and under-reports
--    the moment they forget. It cannot represent the pre-stamp rows at all
--    without inventing a sentinel, because origin_basis is NULL there and a
--    primary key column cannot be NULL — those rows would be summarised
--    nowhere and, by the same EXISTS test, pruned never, which quietly
--    reintroduces the unbounded ledger this migration is capping. Splitting
--    in the columns costs three integers and four numerics per row; splitting
--    in the key costs the meaning of every other column.
--
...
COMMENT ON COLUMN public.job_board_exit_rollup.n_stated IS
  'Exits whose days_on_board was measured from the EMPLOYER''S posted_at AND '
  'carries a duration. The clean population, and the exact denominator of '
  'p50_days_stated and p75_days_stated — the count and the percentile are '
  'computed over one population on purpose, so this can be printed beside '
  'them. `exits` counts a LARGER set (it needs neither a basis nor a '
  'duration), so n_stated / exits is this group''s stated-basis coverage, the '
  'same quantity job_board_closure_rollup publishes as dated_n / fills.';
...
COMMENT ON COLUMN public.job_board_exit_rollup.n_discovered IS
  ...
  'stated no post date, and carries a duration. A LOWER BOUND on tenure, never '
  'a posting age. The exact denominator of p50_days_discovered and '
  'p75_days_discovered, on the same one-population rule as n_stated. These '
  'rows are summarised rather than filtered away because the prune deletes '
  'what this function summarises: dropping them from the rollup would delete '
  'them from history, and an undated posting that aged out is still a ghost.';
