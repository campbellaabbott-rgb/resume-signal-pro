-- A METRO ALIAS MUST MATCH ANY OF ITS NAMES — ON THE RANKED PATH TOO.
--
-- Measured live 2026-08-22, and the two paths disagree:
--   {"location":"bay area"}                  -> San Francisco 40, San Jose 10, Oakland 5
--   {"location":"bay area","q":"engineer"}   -> San Francisco 54, Oakland  1, San Jose 0
-- Adding a search term SHRINKS which cities a metro covers. The browse path
-- binds every expanded name through an OR of ILIKEs; the ranked path takes one
-- text parameter, so the edge function had to pick a single canonical name and
-- the rest of the metro silently vanished.
--
-- WORSE, AND THE REASON THIS IS URGENT: the page now RENDERS the expansion.
-- Both paths emit the same disclosure, so a ranked search prints "Searched
-- 'bay area' as San Francisco, Oakland, San Jose" while the query touched only
-- San Francisco. Before the renderer shipped this was a silent gap; now it is
-- a false statement on screen, which is worse than the gap was.
--
-- THIS IS A REBUILD OF A CHANGE THAT WAS REVERTED FOR AN UNRELATED REASON.
-- 20260820181500 did exactly this and was rolled back by 20260820213000 — not
-- because splitting was wrong, but because that migration was written from a
-- stale repo file, re-issued a fourteen-parameter definition against a live
-- fifteen-parameter one, and left PostgREST with two candidates it refused to
-- choose between. Ranked search returned the ambiguity code for hours. The
-- feature was collateral damage; the design was sound.
--
-- So this file does what that incident's own post-mortem demanded: it is built
-- from the signature that is LIVE RIGHT NOW, confirmed by probing the deployed
-- function rather than by reading the newest-looking migration. Both parameter
-- lists are byte-identical to 20260822171339, which is what is deployed, so
-- nothing is overloaded and nothing is dropped that is still in use.
--
-- WHY A PIPE AND NOT A COMMA. The state aliases contain commas by design — a
-- state maps to its spelled-out name AND to ", TX", because anchoring on the
-- comma is what stops a bare two-letter code matching inside ordinary words
-- ("%CA%" returns 113,223 rows of which ~70% are Canada and "3 LoCAtions").
-- Splitting those on a comma would shred them. A pipe cannot arrive from a
-- visitor either: sanitizeTerm strips | " % _ and backslash from every typed
-- term, so the only thing that can introduce one is our own alias table.
--
