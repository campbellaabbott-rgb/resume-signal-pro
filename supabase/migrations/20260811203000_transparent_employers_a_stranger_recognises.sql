...
-- RE-ASSERTED, NOT ASSUMED. CREATE OR REPLACE preserves existing privileges, so
-- these are strictly redundant today — and that is exactly why they belong in
-- every migration that redefines this function. A future DROP + CREATE would
-- silently restore the PUBLIC default and hand anonymous callers a four-minute
-- aggregate again, and nothing in the diff would look wrong. Stating the
-- security posture beside the definition means the two cannot drift apart.
...
-- No inline recompute; the hourly :07 cron picks this up. A DO block calling
-- refresh_explore_cache() here is what kept two earlier migrations from
