-- "Longest-open postings" tab, implemented honestly. The literal version is
-- structurally impossible on this board: FRESH_WINDOW_DAYS = 30 drops postings
-- older than 30 days at ingest (measured live: 0 of 557k postings carry a
-- stated age > 32d) — the freshness promise and "longest open" cannot coexist
-- on the live table. What we DO own is the stronger bad-practices signal: the
-- closure log records every time a company takes a role down while the same
-- title is still live (superseded = repost churn). Re-listing resets the
-- "posted" date — it is exactly HOW long-open roles are disguised as fresh.
-- Measured live 2026-07-21 (12,995 events, 7 days): BoxLunch re-listed
-- "Part-Time Assistant Manager - Level 2" 289x, Centria "Behavior Technician"
-- 118x, Public Storage one manager role 78x. Real, damning, ours to publish.
-- Copy stays factual (we report measured re-listings, not inferred intent).
...
-- Extend the cache builder with the 'reposters' collection (all others unchanged).
