-- Public aggregate stats over the scan corpus, powering the live data study
-- at /research/ats-score-benchmarks. Original data no competitor or generic
-- chatbot has — the page exists to earn citations and links.
--
-- Privacy: aggregates only. No visitor_id, no ip_country, no per-scan rows;
-- every industry/experience bucket needs n >= 25 before it appears, so no
-- value can be traced to an individual scan.
