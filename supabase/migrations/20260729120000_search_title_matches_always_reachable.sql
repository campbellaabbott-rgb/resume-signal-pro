-- A title match can be unreachable at any offset, because the candidate pool is
-- truncated by RECENCY before anything is ranked.
--
-- search_jobs has two tiers. When fewer than 200 postings match on TITLE it
-- widens to search_tsv (title + description) to rescue thin queries. That wider
-- path builds its candidate pool like this:
--
--     WITH sample AS (
--       SELECT p.id FROM job_board_postings p WHERE p.search_tsv @@ $1 <filters>
--       ORDER BY p.effective_posted DESC
--       LIMIT 3000
--     ) ... ORDER BY ts_rank_cd(p.title_tsv, $1) DESC, ...
--
-- The ranking is correct. The SAMPLE is not: it keeps the 3,000 most recent
-- rows matching title-OR-description, and only then ranks. A posting whose
-- TITLE matches exactly is dropped before ranking if 3,000 description-only
-- matches happen to be newer than it. No offset can reach it, because it was
-- never a candidate.
--
-- Measured on 'Patient Services Assistant': 36 title matches, 12,923
-- description matches, 200 slots requested. Returned 5 of the 36 — 86% of exact
-- title matches lost — with 197 of 200 slots filled by description-only rows.
-- That is the worst failure a job board can have: a posting whose title is
-- literally what the user typed, that search cannot find.
--
-- THE FIX: union the title matches into the pool unconditionally. This tier
-- only runs when title_total < 200, so the title set is small by construction —
-- the LIMIT 500 below is a safety bound that the branch condition already makes
-- unreachable. The recency-ordered description sample is unchanged, so the
-- rescue behaviour and its cost are preserved; the pool simply can no longer
-- exclude an exact title match.
--
-- Everything else is carried over VERBATIM: the same 14-parameter signature,
-- the same RETURNS TABLE, the same filters string and parameter positions, the
-- same two-tier threshold, the same ts_headline snippet, the same ordering and
-- the same LIMIT/OFFSET clamps. Signature-identical is deliberate: replacing a
-- function with a DIFFERENT arity creates an overload rather than a
-- replacement, which is exactly how get_category_fill_speed became an ambiguous
-- PGRST203 earlier today.
...
BEGIN
  ...
  IF tsv_col = 'p.search_tsv' THEN
    RETURN QUERY EXECUTE
      'WITH title_hits AS ('
      -- EVERY title match, unconditionally. This branch only runs when
      -- title_total < 200, so the bound below is never actually reached; it is
      -- there so a future change to the threshold cannot make this unbounded.
      || '  SELECT p.id AS sid FROM public.job_board_postings p WHERE p.title_tsv @@ $1' || filters
      || '  LIMIT 500'
      || '), desc_hits AS ('
      -- The original recency-ordered sample, unchanged: this is the rescue.
