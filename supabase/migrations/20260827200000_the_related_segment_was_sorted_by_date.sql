-- THE RELATED SEGMENT HAD NO RELEVANCE ORDERING AT ALL. IT WAS SORTED BY DATE.
--
-- MEASURED, live, 2026-08-27. Six plain-English searches:
--
--   "help old people at home"      -> Caregiver/Home Health Aide, DSP for 6yr
--                                     old girl, PROJECT MANAGEMENT OFFICER,
--                                     RETAIL SALES ASSOCIATE, COURIER, COURIER
--   "work with my hands outdoors"  -> CUSTOMER SERVICE REPRESENTATIVE, Service
--                                     Foreman-Glazing, ..., MANAGER RETAIL
--                                     DEVELOPMENT                (3,000 related)
--   "drive a truck at night"       -> Lead Truck Mechanic, Late night Delivery
--                                     Driver, CDL A Truck Driver, RAILROAD
--                                     CROSSING TECHNICIAN, RENTAL COORDINATOR
--   "teach kids to swim"           -> six swim instructors           (27 related)
--
-- The junk is not random and it is not a matching failure. websearch_to_tsquery
-- ANDs bare words, so every row shown really does contain all of them — in four
-- thousand characters of description, where "must be 18 years old", "help our
-- customers", "our people" and "work from home" are boilerplate. The AND is
-- doing nothing. The last line of defence is supposed to be the ranking.
--
-- THERE WAS NO RANKING. Every ORDER BY in the union branch reads
--
...
-- Not "a low score" — zero, identically, for every related row. The sort then
-- falls to `p.effective_posted DESC`. The related segment, up to 3,000 rows, was
-- ordered by nothing but recency. "Project Management Officer" beat the
-- caregiving jobs because it was posted more recently. Note the correlation in
-- the measurements: 27 related rows is a clean page, 3,000 is noise. Quality
-- tracks the size of the segment that has no ordering.
--
-- THE FIX IS A TIEBREAK, NOT A NEW TIER. Rank the title against the OR form of
-- the same query. A row whose title carries some of the words sorts above one
-- whose title carries none. That is precisely what rerankWindow already does in
-- the edge function — and why the top two rows above ARE the sensible ones. The
-- edge can only reorder the twenty rows SQL hands it, and SQL was handing it the
-- twenty most recent of three hundred and ninety. The signal is right; it was
-- being applied one stage too late, to a window chosen by date.
--
-- THE TITLE SEGMENT IS BYTE-IDENTICAL. The new key is `CASE WHEN title_tsv @@ $1
-- THEN 0 ELSE ...`, a constant for every title match, so ties among them still
-- break on effective_posted exactly as before. Today's weights-only rank change
-- (this migration's predecessor) is untouched; this only fills in a sort key
-- that was constant-zero.
--
-- COSTS NOTHING. title_tsv is a short vector already read by the existing ORDER
-- BY and always stored inline. Ranking the DESCRIPTION vector instead would mean
-- detoasting search_tsv for up to 3,000 rows per query, which is exactly the
-- kind of trade this board has paid for before.
--
-- WHAT THIS DOES NOT FIX, stated plainly: desc_hits is still the 3,000 most
-- RECENT description matches, so for a query with more than 3,000 the pool is
-- still chosen by date and a better match outside it stays invisible. Ordering
-- that pool by relevance means ranking every match, not three thousand — a
-- different and much more expensive change. Ranking a recency-truncated pool is
-- strictly better than ranking nothing, which is what happens today.
--
-- NOT one of the four discriminators refuted with data in
-- 20260822040000 and after. Those all asked WHICH VECTOR to match against —
-- title vs search_tsv — and none separated cleanly. This asks nothing about
-- matching. The tier choice, the row set, and both counts are untouched; only
-- the order of rows already being returned changes.
--
-- VERIFIED BY EXECUTION, not by reading. Both this migration and its
-- predecessor were applied to a real Postgres (pglite) over a stand-in schema
-- carrying the same generated title_tsv/search_tsv/effective_posted columns,
-- and queried with four rows built to the shape measured above: two caregiving
-- jobs posted 5 and 9 days ago, and two boilerplate matches posted in the last
-- hour.
--
--   q = "help old people at home"
--   BEFORE  Project Management Officer, Courier, Caregiver/Home Health Aide,
--           Home Care Assistant                        <- exactly date order
--   AFTER   Caregiver/Home Health Aide, Home Care Assistant,
--           Project Management Officer, Courier
--
-- The two right answers are the two OLDEST rows, which is why recency put them
-- last. Both builds return the same four rows and the same counts (n=5,
-- capped=false): only the order moved.
--
--   q = "home care assistant" over three same-titled rows at 1, 3 and 7 days
--   BEFORE  t2 t1 t3      AFTER  t2 t1 t3      <- identical, newest first
--
-- The title segment is unchanged, as the CASE guarantees. Quoted phrases, a
-- single word, a stopwords-only query and an empty string all return without
-- raising in both builds — the querytree fallback paths.

-- CONTRACTS CARRIED FORWARD UNCHANGED, because this file is now the live
-- definition of all three functions and has to state what it inherits:
...
--     Both functions carry it — search_jobs at $3, count_jobs_capped at $2 — or
--     the rows come from Philadelphia while the count searches for the literal
--     "Philly|Philadelphia" and matches nothing.
--   * p_work_mode is a comma list validated against {remote,hybrid,onsite}.
--   * title ranking stays weights-only, ARRAY[0,0,0,1] over title_tsv: the
--     normalization variants were measured and rejected — norm 2 divides by the
--     whole vector length including company lexemes, so two identical
--     "Registered Nurse" titles scored 0.3333 and 0.1250 purely because one
--     employer's name is longer.
--
-- DROP + CREATE with IDENTICAL signatures, all three re-issued together, for the
-- standing reason: an old overload left in ANY database makes PostgREST answer
-- PGRST203 to every call. Bodies were extracted programmatically from
-- 20260827170000 and patched at the ordering sites only, never retyped. No
-- signature changes: q_or is derived inside the function, so the edge function
-- needs no change and the two deploys are independent.
