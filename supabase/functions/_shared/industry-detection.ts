const KNOWN_EMPLOYERS: Record<string, string> = {
  ...
  'compass real estate': 'real_estate',
  ...
  'genentech': 'biotech', 'amgen': 'biotech', 'thermo fisher': 'biotech',
  'delta air lines': 'aviation', 'united airlines': 'aviation', 'american airlines': 'aviation',
  'southwest airlines': 'aviation', 'airbus': 'aviation',
  'exxonmobil': 'energy', 'halliburton': 'energy',
  'schlumberger': 'energy', 'nextera': 'energy', 'duke energy': 'energy', 'sunrun': 'energy',
  'cvs pharmacy': 'pharmacy', 'rite aid': 'pharmacy',
  'banfield': 'veterinary', 'vca animal': 'veterinary',
  ...
  'cargill': 'agriculture', 'archer daniels': 'agriculture',
  ...
  'riot games': 'gaming', 'activision': 'gaming',
  'blizzard': 'gaming', 'electronic arts': 'gaming', 'ubisoft': 'gaming',
  'etsy': 'ecommerce', 'wayfair': 'ecommerce', 'chewy': 'ecommerce',
...
function extractSections(resumeText: string): {
  ...
  const BLOCK_WEIGHTS = [2.0, 1.5, 1.0, 0.6];
...
function calculateIndustryScore(sections: ReturnType<typeof extractSections>, industry: string): {
  ...
  sections.jobTitles.forEach((title, index) => {
    ...
    for (const industryTitle of keywords.titles) {
      ...
      if (matches) {
        score += SECTION_WEIGHTS.jobTitle * 2 * recencyMultiplier;
...
export function detectIndustry(
    ...
    if (expHeading !== -1) {
      ...
      for (const entry of scores) {
        ...
        for (const title of kw.titles) {
          if (recentBlock.includes(title)) recentHits += 2;
        }
  ...
  if (roleLocks) {
    ...
    if (lockedEntry) {
      lockedEntry.score = Math.max(lockedEntry.score, 40);
