...
interface AIGenerationTrendChartProps {
  className?: string;
}

type TimeRange = '24h' | '7d';

export function AIGenerationTrendChart({ className }: AIGenerationTrendChartProps) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.error('Failed to fetch AI generation trends:', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const totalGenerations = data.reduce((sum, d) => sum + d.total_generations, 0);
  const totalFailed = data.reduce((sum, d) => sum + d.failed_generations, 0);
  const avgSuccessRate = totalGenerations > 0 
    ? ((totalGenerations - totalFailed) / totalGenerations * 100).toFixed(1) 
    : '0.0';
  const avgDuration = data.length > 0
    ? Math.round(data.reduce((sum, d) => sum + (d.avg_duration_ms || 0), 0) / data.length)
    : 0;
        ...
        {payload.map((entry: any, index: number) => (
          <div key={index} className="flex items-center justify-between gap-4">
            ...
            <span className="font-mono">
              {entry.name.includes('Duration') ? `${Math.round(entry.value)}ms` : entry.value}
            </span>
          </div>
        ))}
      </div>
      ...
      <CardContent>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            <ResponsiveContainer width="100%" height={140}>
              <AreaChart data={data}>
                ...
                <Legend />
                <Area type="monotone" dataKey="successful_generations" name="Success" stroke="hsl(var(--chart-1))" fill="hsl(var(--chart-1))" fillOpacity={0.3} />
                <Area type="monotone" dataKey="failed_generations" name="Failed" stroke="hsl(var(--destructive))" fill="hsl(var(--destructive))" fillOpacity={0.3} />
              </AreaChart>
            </ResponsiveContainer>
            ...
            <ResponsiveContainer width="100%" height={100}>
              <AreaChart data={data}>
                ...
                <YAxis tick={{ fontSize: 11 }} />
                <Tooltip content={<CustomTooltip />} />
                <Area type="monotone" dataKey="avg_duration_ms" name="Avg Duration" stroke="hsl(var(--chart-2))" fill="hsl(var(--chart-2))" fillOpacity={0.2} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        ) : (
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p>No AI generation data for this period</p>
          </div>
        )}
      </CardContent>
    </Card>
