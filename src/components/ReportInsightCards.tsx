...
export function ShareScoreCard({
  ...
  industry: string;
  percentile?: number;
  /** When the same candidate scanned before, the card becomes a before/after
   *  ("58 → 74") — measured improvement is what people actually share. */
  previousScore?: number;
}) {
  const isDelta = typeof previousScore === "number" && previousScore !== atsScore;
  ...
  const draw = (): HTMLCanvasElement | null => {
    ...
    if (isDelta) {
      ctx.fillText("I fixed my resume.", 560, 180);
      const delta = atsScore - (previousScore as number);
      ctx.font = "bold 84px system-ui, sans-serif";
      ctx.fillStyle = "#64748b";
      ctx.fillText(String(previousScore), 560, 300);
      const beforeW = ctx.measureText(String(previousScore)).width;
      ctx.fillStyle = "#94a3b8";
      ctx.fillText(" → ", 560 + beforeW, 300);
      const arrowW = ctx.measureText(" → ").width;
      ctx.fillStyle = scoreColor;
      ctx.fillText(String(atsScore), 560 + beforeW + arrowW, 300);
      ctx.font = "600 34px system-ui, sans-serif";
      ctx.fillStyle = delta > 0 ? "#34d399" : "#f87171";
      ctx.fillText(`${delta > 0 ? "+" : ""}${delta} points — same rubric both times`, 560, 370);
    } else {
  ...
  // Native share sheet (mobile: one tap to WhatsApp/Instagram/X). Falls back
  // to download where the Web Share API can't share files.
  const share = () => {
    const canvas = draw();
    if (!canvas) return;
    canvas.toBlob((blob) => {
      if (!blob) return download();
      const file = new File([blob], `resume-score-${atsScore}.png`, { type: "image/png" });
      ...
      if (nav.share && nav.canShare?.({ files: [file] })) {
        nav.share({
          ...
          text: isDelta
            ? `Took my resume from ${previousScore} to ${atsScore} — free scan at resumebooster.work`
            : `My resume scored ${atsScore}/100 — free scan at resumebooster.work`,
    ...
    <div className="rounded-2xl border border-border bg-card p-5 sm:p-6 flex flex-col sm:flex-row items-start sm:items-center gap-3">
      ...
      <div className="flex-1">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h3 className="font-semibold text-foreground text-sm">{isDelta ? "Share your before/after" : "Share your score"}</h3>
        </div>
        <p className="text-xs text-muted-foreground">
          {isDelta
            ? `A clean ${previousScore} → ${atsScore} card — no resume content on it, just the improvement.`
            : "A clean score card image — no resume content on it, just the number."}
        </p>
      </div>
      <div className="flex gap-2 shrink-0">
        <button onClick={share} className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-primary text-primary-foreground text-xs font-semibold hover:bg-primary/90 transition-colors">
          <Share2 className="w-3.5 h-3.5" />
          Share
        </button>
        <button onClick={download} className="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-border text-foreground text-xs font-semibold hover:border-primary/50 transition-colors">
