...
describe("the scorer in its own isolate", () => {
  ...
  it("(6) lastUpsertError rides the slice note onto slice_stats; chainKick exposes at", () => {
    expect(BOARD).toMatch(/lastUpsertError: string \| null; heapStopped: boolean; wallStopped: boolean \} \| null = null;/);
    // .41 appended heapStopped to the note (the-budget-counted-the-wrong-thing).
    expect(BOARD).toMatch(/hit: budgetSkipped\.length > 0, lastUpsertError, heapStopped, wallStopped \};/);
