...
describe("nothing bounded the sum", () => {
  it("has a per-slice budget in POSTINGS", () => {
    expect(CODE).toMatch(/const SLICE_POSTING_BUDGET = 4_000;/);
  });
  ...
  it("records the outcome where status already looks", () => {
    const note = CODE.indexOf("sliceBudgetNote = { fetched: fetchedInSlice, skipped: budgetSkipped.length, hit: budgetSkipped.length > 0, lastUpsertError, heapStopped, wallStopped, sizeStopped, boardBudget };");
